...
class RetrievalFramework:
    ...
    # Web 接口用的单次检索方法 
    def search(self, query: str, top_k: int = 10, with_instruction: bool = False):
        """
        对单条 query 做检索，返回 top_k 个候选数据集。
        返回结构是 list[dict]，直接给 Flask 用。

        Args:
            query: 纯文本查询（来自前端输入框）
            top_k: 返回前 k 个候选
            with_instruction: 是否拼接 instruction（沿用原有逻辑）
        """
        if self.current_retriever is None:
            raise RuntimeError("No retriever initialized. Call initialize_retriever() first.")

        # 构造一个“伪 query 字典”，让 DataLoader 正常工作
        # DataLoader.get_query_text(query_dict) 一般会从 "query" 字段里取文本
        query_dict = {"query": query}

        # 直接复用你已经写好的单条检索逻辑
        results_pairs = self.retrieve_single_query(
            query=query_dict,
            with_instruction=with_instruction,
            top_k=top_k,
        )
        # results_pairs: List[Tuple[dataset_dict, score]]

        formatted = []
        for dataset, score in results_pairs:
            formatted.append(
                {
                    "score": float(score),
                    "paper_link": dataset.get("Paper Link", ""),
                    "dataset_link": dataset.get("Dataset Link", ""),
                    "description": dataset.get("Dataset Description", ""),
