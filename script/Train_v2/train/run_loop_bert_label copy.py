...
from utils.utils_loop import (
    ...
    _LLM_SEM, _post_qwen_with_retry,     # ← 新增
)
...
if not hasattr(CONFIG, "LATEX_LLM_DISABLED"):
    # 1=完全禁用 LLM；0=保持现状（规则优先，LLM 兜底）
    import os
...
if not hasattr(CONFIG, "APPEND_LINK_SUMMARIES_TO_DESC"):
    setattr(CONFIG, "APPEND_LINK_SUMMARIES_TO_DESC", True)
else:
    CONFIG.APPEND_LINK_SUMMARIES_TO_DESC = True  # 你想开就强制开
...
def _urls_to_domains(text: str) -> str:
    """把文本里的 https://... 变成裸域名（open-reco-bench.ai），避免删掉后语义断裂。"""
    def _rep(m):
        try:
            dom = urlsplit(m.group(0)).netloc.lower()
            return dom if dom else m.group(0)
        except Exception:
            return m.group(0)
    return _URL_RE.sub(_rep, text)

def _smart_truncate_sentence(text: str, max_len: int = 220) -> str:
    return (text or "").strip()


def _sanitize_llm_summary_line(s: str, max_len: int = 220) -> str:
    """把 LLM/聚合出的摘要清洗成可读的一句或几句自然语言，并做智能截断。"""
    s = (s or "").replace("\n", " ").strip()
    s = re.sub(r"\s+", " ", s)
    # 去掉“提示词/日志/代码”痕迹
    ...
    if _looks_like_code_or_log(s):
        return ""
    # ★ 关键：URL -> 域名，避免“available at”后面真空
    s = _urls_to_domains(s)
    # 太短/无信息就丢弃
    if len(s) < 18:
        return ""
    return s
...
# === 新增：全局可配置的 LLM 输入上限（字符预算，留 10% 余量） ===
DEFAULT_LLM_CHAR_LIMIT = int(getattr(CONFIG, "MAX_LLM_INPUT_CHARS", 50000))
DEFAULT_LLM_CHAR_LIMIT = max(1000, int(DEFAULT_LLM_CHAR_LIMIT * getattr(CONFIG, "LLM_OVERHEAD_RATIO", 0.9)))
...
def collect_dataset_description(json_path: str, max_chars: int = 2000) -> str:
    try:
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        pos_texts = []
        for item in data.get("data", []):           # 遍历所有 section/段落
            for s in item.get("sentences", []):
                lab = int(s.get("label", s.get("predicted_label", 0)) or 0)
                # 兼容两种字段名：原始解析常用 text，评估阶段有时叫 sentence_text
...
def label_sentences_via_bert(
    ...
    try:
        ...
        missing, unexpected = model.classifier.load_state_dict(state, strict=True)
        assert not missing and not unexpected, f"classifier head mismatch: missing={missing}, unexpected={unexpected}"
...
def _final_dataset_gate(pairs):
    """pairs: List[(url_norm, context_dict)]，保留结构化校验通过的条目。"""
    ok = []
    for u, c in pairs:
        if _is_structural_dataset_url(u):
            ok.append((u, c))
        else:
            c.setdefault("debug", {}).setdefault("reasons", []).append("structural_reject")
    return ok
...
def _write_primary_dataset_item(rec: dict, item: dict):
    ...
    FINAL_DESC_MAX_LEN = int(getattr(CONFIG, "FINAL_DESC_MAX_LEN", 300))

    # 1) 先用传入的 desc/why
    raw_line = (item.get("desc") or item.get("why") or "").strip()
    line = _sanitize_llm_summary_line(_urls_to_domains(raw_line), max_len=FINAL_DESC_MAX_LEN)

    # 2) 若为空或只是裸 URL，则回退到论文级描述（先 URL->域名，再清洗+智能截断）
    if (not line) or (_norm_url(line) == _norm_url(u)) or _URL_RE.fullmatch(line):
        paper_desc = (rec.get("dataset_description") or "").strip()
        if paper_desc:
            paper_desc = _urls_to_domains(paper_desc)
            line = _sanitize_llm_summary_line(paper_desc, max_len=FINAL_DESC_MAX_LEN)

    # 3) 仍为空则兜底：host — anchor
    if (not line) or (_norm_url(line) == _norm_url(u)):
        host = urlsplit(u).netloc.lower()
        anchor = (item.get("why") or "").strip() or "dataset link"
        line = f"{host} — {anchor}"
...
def _run_latex_stage(cat: str, results: list):
    ...
    for rec in results:
        ...
        if not cands:
            core_logger.info("[LaTeX] %s no structural dataset-like links.", aid)
            continue

        # 6) 先规则直出；不够强再 1-shot LLM 甄别
        # 在前 topk 里优先挑可信主机（HF/Zenodo/Kaggle/...）
        ...
        if preferred and not getattr(CONFIG, "DISABLE_PREFERRED_PICK", False):
            ...
            picked = _choose_primary_rule_only(cands)


        # === 新增：当禁用 LLM 时，强制走“规则-only”路径并跳过 LLM ===
        if getattr(CONFIG, "LATEX_LLM_DISABLED", False):
            if not picked and getattr(CONFIG, "RULE_ONLY_ALWAYS_TOP1", True) and cands:
                top1 = max(cands, key=lambda x: x["score"])
                picked = {"url": top1["url"], "why": top1.get("anchor", ""), "primary_score": float(top1.get("score", 0))}
                core_logger.warning("[LaTeX][fallback-top1] %s no strong/LLM pick; fallback to top1 %s",
                                    aid, top1["url"])
                
            if picked:
                _write_primary_dataset_item(
                    rec, {"url": picked["url"], "desc": "", "why": picked.get("why", ""),
                        "primary_score": picked["primary_score"], "llm_conf": 0.0}
                )
                core_logger.info("[LaTeX][rule-only] %s primary: %s (score=%.2f)",
                                aid, picked["url"], picked["primary_score"])
            else:
                core_logger.info("[LaTeX][1shot] %s no primary selected.", aid)
                # <<< 新增：始终允许兜底为 Top1（默认开启，可通过配置关闭） >>>
                if getattr(CONFIG, "FALLBACK_TOP1_ALWAYS", True) and cands:
                    ...
                    _write_primary_dataset_item(
                        rec,
                        {
                            ...
                            "desc": "",
                            "why": top1.get("anchor", ""),
                            "primary_score": float(top1.get("score", 0)),
                            "llm_conf": 0.0,
                        },
                    )
                    core_logger.warning(
                        "[LaTeX][fallback-top1] %s no strong/LLM pick; fallback to top1 %s",
                        aid, top1["url"]
                    )
            continue 
        ...
        # === 原有逻辑：规则失败时再 LLM 兜底 ===
        if picked:
            _write_primary_dataset_item(rec, {"url": picked["url"], "desc": "", "why": picked.get("why",""),
                                            "primary_score": picked["primary_score"], "llm_conf": 0.0})
            core_logger.info("[LaTeX][rule] %s primary: %s (score=%.2f)", aid, picked["url"], picked["primary_score"])
        else:
            core_logger.info("[LaTeX][1shot] %s no primary selected.", aid)
            # === 新增：LLM 未选出时，统一回退 Top1 ===
            if getattr(CONFIG, "FALLBACK_TOP1_ALWAYS", True) and cands:
                # 前面已排序，这里直接取 cands[0]
                top1 = cands[0]
                _write_primary_dataset_item(
                ...
                core_logger.warning(
                    "[LaTeX][fallback-top1] %s no LLM pick; fallback to top1 %s",
                    aid, top1["url"]
                )
        ...
        core_logger.info(
            ...
            aid, len(links_detail), len(cands), "yes" if picked else "no"
        )
...
SLOW = {
    ...
    "latex_build_windows_ms":     6_000,   # 这里只是占位，若你也想包窗口函数可加
    "latex_score_filter_ms":      4_000,
    "latex_pick_primary_ms":      3_000,   # 也用于 LLM 调用
}
...
def main_loop():
    ...
    while True:
        ...
        # —— 本轮“已处理”的缓存，防止同一轮跨分类重复 —— 
        processed_ids: set = set()

        # —— 全局 seen 快照（把所有 cat 的 seen 合起来） ——
...
if __name__ == "__main__":
    CONFIG.LAST_N_DAYS = 60
    setup_logging()
    install_perf_probes()   # ← 加这一行
