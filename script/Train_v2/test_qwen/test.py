...
class ResearchDatasetEvaluator:
    def __init__(self):
        ...
        base_dir = Path("/home/kzlab/muse/Savvy/Data_collection/script/Train_v2/train/outputs/api")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.results_dir = base_dir / f"results_{timestamp}"
        self.papers_dir = self.results_dir / "papers"
        self.final_results_file = self.results_dir / "final_evaluation_results.json"
        self.interim_results_file = self.results_dir / "interim_results.json"
    ...
    def evaluate_all_papers(self, papers: List[Dict]) -> Dict:
        ...
        if all_true_labels and all_predicted_labels:
            ...
            final_results = {
                ...
                "paper_results_directory": str(self.papers_dir)
            }
...
def main():
    ...
    if not results:
        print("\n=== 总体评估结果 ===")
        print("没有得到任何有效结果（可能是上游报错或标签数量不匹配）。")
        return  

    print("\n=== 总体评估结果 ===")
    print(f"评估论文总数: {results['total_papers']}")
    print(f"已处理论文数: {results['processed_papers']}")
    m = results['overall_metrics']
    print(f"整体准确率: {m['accuracy']:.4f}")
    print(f"整体精确率: {m['precision']:.4f}")
    print(f"整体召回率: {m['recall']:.4f}")
    print(f"整体F1分数: {m['f1']:.4f}")
