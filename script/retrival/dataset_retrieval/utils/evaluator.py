...
class Evaluator:
    ...
    def evaluate_retrieval(self, query: Dict[str, Any],
        ...
        retrieved_links = [result[0].get("Dataset Link", "") for result in retrieval_results]
    ...
    def _get_relevant_links(self, query: Dict[str, Any]) -> Set[str]:
        ...
        ground_truth_link = self._get_ground_truth_link(query)
        if ground_truth_link:
            relevant.add(ground_truth_link)

        metadata_link = query.get("metadata", {}).get("dataset_link")
        if metadata_link:
            relevant.add(metadata_link)

        for link in query.get("positives", []):
            if link:
                relevant.add(link)
